...
class RAGSystem:
    ...
    def get_system_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "query_understanding_enabled": self.enable_query_understanding,
    ...
    def search_documents(self, query: str, top_k: int = 10) -> List[Document]:
        ...
        # Use query understanding to extract keywords if enabled
        search_query = query
        if self.enable_query_understanding:
            keywords = self.query_understanding.extract_keywords(query, self.get_history())
            print(f"Extracted keywords: {keywords}")
            search_query = keywords
        
        # Use search query for vector search
        docs = self.vectorstore.similarity_search(search_query, k=top_k)
    ...
    def generate_answer(self, query: str, top_docs: List[Document]) -> str:
        """
        Generate answer using query, documents, and conversation history
        
        Args:
            query: Current user query
            top_docs: List of relevant documents
        ...
        history_text = ""
        for turn in self.get_history()[:-1]:  # Exclude current query
            role = turn['role']
        ...
        prompt = f'''
You are a helpful AI assistant. Please provide a comprehensive answer to the query using the provided context and conversation history.
Make sure your answer is consistent with previous responses and builds upon the conversation context when relevant.
...
Answer: '''
        ...
        answer = self.llm_client.call_llm_api(prompt)
    ...
    def query(self, user_query: str, top_k: int = 10, 
        ...
        try:
            ...
            # Step 1: Determine if the query needs RAG processing
            needs_rag = self.query_understanding.need_rag(user_query, self.get_history())
            print(f"Query requires document search: {needs_rag}")
            
            if not needs_rag:
                # Handle queries that don't require document search
                history_text = ""
                for turn in self.get_history()[:-1]:  # Exclude current query
                    role = turn['role']
                    content = turn['content']
                ...
                prompt = f'''
You are a helpful AI assistant. Please provide a direct and concise answer to the following query.
If you find that the query actually requires searching through documents or specific information lookup, indicate that document search is needed.
...
Answer: '''
                direct_answer = self.llm_client.call_llm_api(prompt)
                
                # Add assistant response to history
                self.add_to_history('assistant', direct_answer)
